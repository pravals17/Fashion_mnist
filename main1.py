...
kf = KFold(n_splits = 5)
for train_index, test_index in kf.split(full_train_images):
    ...
    with tf.Session() as session:
        ...
        train_labels, test_labels = full_train_labels[train_index], full_train_labels[test_index]
        patience_param = 0
...
avg_epoch_earlydropping = math.ceil(sum(epoch_saved)/5)
training_set, test_set = util.split_rows(full_train_data, 0.8)   
training_data = np.hsplit(training_set, [784, 794]) 
train_images = training_data[0]
train_labels = training_data[1]
...
with tf.Session() as session:
    ...
    for epoch in range(avg_epoch_earlydropping):
        print("Epoch After KFold:" + str(epoch))
        train(train_images, train_labels)
